	...
	function wplook_bar_menu() {
		...
		$wp_admin_bar->add_menu(
			array(
				...
				'href' => 'https://wplook.com/docs/?utm_source=Support&utm_medium=link&utm_campaign=Health-Medical',
...
if ( ! function_exists( 'wplook_breadcrumbs' ) ) {
	function wplook_breadcrumbs() {
		...
		if ( is_category() ) {
			...
			echo $delimiter . ' <span>' . get_the_title( $pid ) . '</span>';
		} elseif ( is_tax() ) {
...
if( !function_exists( 'wplook_flush_rewrite' ) ) {
	 ...
	 * @since 1.0.5
	 */
	function wplook_flush_rewrite() {
		flush_rewrite_rules();
	}

	add_action( 'after_switch_theme', 'wplook_flush_rewrite' );
	add_action( 'switch_theme', 'wplook_flush_rewrite' );
